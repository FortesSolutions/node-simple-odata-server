...
ODataServer.prototype.handle = function (req, res) {
  ...
  // Determine base serviceUrl once if not set: use constructor value if provided, otherwise infer from request
  if (!this.cfg.serviceUrl) {
    const base = this.serviceUrl ? this.serviceUrl : (req.protocol + '://' + req.get('host'));
    const originalPath = (req.originalUrl || '').replace(new RegExp(escapeRegExp(req.url) + '$'), '');
    this.cfg.serviceUrl = base + originalPath;
  }
