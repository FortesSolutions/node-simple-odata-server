...
ODataServer.prototype.handle = function (req, res) {
  if (!req.protocol && !this.serviceUrl) {
  ...
  // Rebuild serviceUrl on each request so @odata.context reflects the mounted path
  this.cfg.serviceUrl = this._buildServiceUrl(req);

  // Ensure serviceUrl always ends with a single slash (used for @odata.context etc.)
  const trimmedServiceUrl = this.cfg.serviceUrl.replace(/\/+$/, '');
  this.cfg.serviceUrl = `${trimmedServiceUrl}/`;

  // Derive router prefix from the mount path (originalUrl minus url), which matches framework behavior
  const mountPath = (req.originalUrl || '/').replace(new RegExp(escapeRegExp(req.url) + '$'), '');
...
ODataServer.prototype._buildServiceUrl = function (req) {
  const base = this.serviceUrl ? this.serviceUrl : (req.protocol + '://' + req.get('host'));
  const mountPath = (req.originalUrl || '/').replace(new RegExp(escapeRegExp(req.url) + '$'), '');
  const normalizedMountPath = mountPath.replace(/\/+$/, '');
  const servicePath = normalizedMountPath || '/';
...
ODataServer.prototype._initializeRoutes = function () {
  const self = this;
  this.router.get('/([\$])metadata', async function (req, res) {
    const model = await self.getModel(req);
  ...
  this.router.get('/:collection/([\$])count', async function (req, res) {
    req.params.$count = true;
    const model = await self.getModel(req);
    query({ ...self.cfg, model }, req, res);
  });
  this.router.get('/:collection\\(:id\\)', async function (req, res) {
    const model = await self.getModel(req);
    query({ ...self.cfg, model }, req, res);
  });
  this.router.get('/:collection', async function (req, res) {
    const model = await self.getModel(req);
    query({ ...self.cfg, model }, req, res);
  });

  const rootHandler = async function (req, res) {
    const model = await self.getModel(req);
    const result = collections({ model, serviceUrl: self.cfg.serviceUrl });
  ...
  // Support instance root with and without trailing slash
  ...
  if (this.cfg.cors) {
    this.router.options('/([\\$\\S]*)', function (req, res) {
      res.statusCode = 200;
...
ODataServer.prototype.executeQuery = function (col, query, req, cb) {
  ...
  this.cfg.beforeQuery(col, query, req, function (err) {
    ...
    self.cfg.query(col, query, req, function (err, result) {
      if (err) {
        return cb(err);
      }

      self.cfg.afterRead(col, result, req);
      cb(null, result);
    });
  });
};
...
ODataServer.prototype.model = function (model) {
  if (typeof model === 'function') {
    this.cfg.model = model.bind(this);
  } else {
...
ODataServer.prototype.getModel = async function (req) {
  if (typeof this.cfg.model === 'function') {
    return await this.cfg.model(req);
  }
...
/**
 * Prune results according to $select and $expand
 *
 * @param collection
 * @param res
 * @param req
 ...
 */
ODataServer.prototype.pruneResults = async function (collection, res, req) {
  prune(await this.getModel(req), collection, res);
};

ODataServer.prototype.base64ToBuffer = async function (collection, doc, req) {
...
ODataServer.prototype.bufferToBase64 = async function (collection, result, req) {
  const model = await this.getModel(req);
  const entitySet = model.entitySets[collection];
  const entityType = model.entityTypes[entitySet.entityType.replace(model.namespace + '.', '')];
...
ODataServer.prototype.setCfg = function (name, value) {
  this.cfg[name] = value;
};
