...
const parser = require('@jsreport/odata-parser')
...
module.exports = function (cfg, req, res) {
  ...
  if (_url.search) {
    ...
    if (encodedQS) {
      try {
        ...
        console.error('Unable to parse', encodedQS, e)
      }
    }
    if (query.$count) {
  ...
  cfg.executeQuery(queryOptions.collection, queryOptions, req, function (err, result) {
    ...
    cfg.pruneResults(queryOptions.collection, out.value, req)

    cfg.bufferToBase64(queryOptions.collection, out.value, req)
