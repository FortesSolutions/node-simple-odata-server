...
module.exports = function (cfg, req, res) {
  ...
  cfg.executeQuery(queryOptions.collection, queryOptions, req, function (err, result) {
    ...
    if (result.nextSkipToken) {
      // Skiptoken-based paging: emit $skiptoken in @odata.nextLink; omit @odata.count (intentional).
      delete _url.query.$skip;
      delete _url.query.$top;
      _url.query.$skiptoken = result.nextSkipToken;
      delete _url.search;
      out['@odata.nextLink'] = cfg.serviceUrl + url.format(_url).replace(/^\/+/, '');
    } else {
