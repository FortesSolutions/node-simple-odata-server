...
const parser = require('../lib/odata-parser');
...
describe('odata.parser grammar', function () {
  ...
  it('should parse $filter with logical and comparison operators', function () {
    const ast = parser.parse('$filter=Price gt 10 and Price lt 20');

    assert.deepEqual(ast.$filter, {
      type: 'and',
      left: {
        type: 'gt',
  ...
  it('should parse $select with international characters in identifiers', function () {
    const ast = parser.parse('$select=Käufer,ΝόμιμοΌνομα');

    assert.equal(ast.$select[0], 'Käufer');
    assert.equal(ast.$select[1], 'ΝόμιμοΌνομα');
  });

  it('should parse $filter with international characters in identifier path', function () {
    const ast = parser.parse('$filter=Käufer/ΝόμιμοΌνομα eq \'Hans\'');

    // Shape depends on the grammar; just sanity-check that it parsed without error
    assert.ok(ast.$filter);
  });
});
